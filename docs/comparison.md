...
| ~~Documents~~                                        | **Shipped.** Invoices, customs declarations and bills of lading attached to the entry or shipment they support; stored once per tenant by SHA-256 under the same row-level security, typed from their bytes, never deleted. [ADR 24](adr/0024-documents.md)                                                                |
| ~~Bank reconciliation and feeds~~                    | **Shipped.** Statements imported from CSV (English, Vietnamese and Japanese headers) or pushed by a feed, stored once, matched one to one with postings of the same amount — the database refuses anything else — with unbooked fees and interest booked straight from the line. [ADR 25](adr/0025-bank-reconciliation.md) |
| ~~Vietnam's statutory statements~~                   | **Shipped.** Mẫu B01-DN and B02-DN (Thông tư 200), B01a-DNN and B02-DNN (Thông tư 133), filled from the ledger with the comparative column, every account placed or listed, exported as CSV. [ADR 26](adr/0026-statutory-statements.md)                                                                                    |
| ~~Vietnamese e-invoices~~                            | **Shipped (document).** The invoice XML in the national format for each sale and an adjustment invoice for each credit note, numbered without gaps within a series the database checks against the year. Signing and transmission go through the business's licensed provider. [ADR 27](adr/0027-vietnam-e-invoices.md)    |
...
| ~~Supplier returns~~                                 | **Shipped.** Goods leave the delivery they came in at what it carries them at; the supplier's account comes down by its own price at the lot's rate; freight and duty nobody refunds go to an expense; local input VAT is reversed on the month's return. [ADR 23](adr/0023-supplier-returns.md)                           |
